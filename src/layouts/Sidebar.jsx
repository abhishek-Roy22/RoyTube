...
const Sidebar = () => {
  return (
    <>
      <aside
        className={`sticky top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col hidden`}
      >
      ...
      <aside
        className={`w-56 h-screen hidden lg:sticky absolute top-20 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 lg:flex lg:ml-4`}
      >
        <div className="lg:hidden pt-2 pb-4 sticky top-0 bg-white">
          <PageHeaderFirstSectin />
        </div>
        <LargeSidebarSection>
...
function LargeSidebarItem({ Icon, title, url, isActive = false }) {
  return (
    ...
    >
      {Icon && <img src={Icon} className="w-6 h-6 rounded-full" />}
